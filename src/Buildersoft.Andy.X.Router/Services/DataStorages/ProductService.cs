...
namespace Buildersoft.Andy.X.Router.Services.DataStorages
{
    public class ProductService
    {
        ...
        public ProductService(IHubContext<DataStorageHub, IDataStorageHub> hub, IHubRepository<DataStorage> dataStorageRepository)
        {
            _hub = hub;
            _dataStorageRepository = dataStorageRepository;
        }

        public async Task CreateProductAsync(ProductDetail product)
        {
            foreach (var storage in _dataStorageRepository.GetAll())
            {
                await _hub.Clients.Client(storage.Key).ProductCreated(product);
            }
        }
