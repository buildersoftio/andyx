...
namespace Buildersoft.Andy.X.Router.Services.DataStorages
{
    public class TenantService
    {
        ...
        public TenantService(IHubContext<DataStorageHub, IDataStorageHub> hub, IHubRepository<DataStorage> dataStorageRepository)
        {
            _hub = hub;
            _dataStorageRepository = dataStorageRepository;
        }

        public async Task CreateTenantAsync(TenantDetail tenant)
        {
            foreach (var storage in _dataStorageRepository.GetAll())
            {
                await _hub.Clients.Client(storage.Key).TenantCreated(tenant);
            }
        }
    }
}
