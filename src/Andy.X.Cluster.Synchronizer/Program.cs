...
namespace Andy.X.Cluster.Synchronizer
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var logSink = new LoggerSink(ConfigurationLocations.ClusterSyncLoggingFile(node.NodeId, DateTime.Now));
            ...
            Logger.Log($"Cluster Synchronizer for '{node.NodeId}' and '{node.Host}:{node.Port}' is online");
